...
3. A. Trigger the ADC and feed to FIR filter (by FMAC) and apply either LPF and HPF and send the data out to DAC  
3. B. Feed data to FMAC manually (Polling), this is useful if data is not coming from ADC but from other devices like accelerometer, vibrations, gyro etc.   
